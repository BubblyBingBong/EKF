...
def runIteration(newDt, localization_mode, add_errors):
    ...
    for _ in range(iterations):
        ...
        angleSum += (
            (actual_robot.getPrevState()[2] - ekf_robot.getPrevState()[2]) ** 2
        ) / (iterations * 1.0)
        xySum += distance(
            actual_robot.getPrevState()[:2], ekf_robot.getPrevState()[:2]
        ) / (iterations * 1.0)
        xSum += (
            (actual_robot.getPrevState()[0] - ekf_robot.getPrevState()[0]) ** 2
        ) / (iterations * 1.0)
        ySum += (
            (actual_robot.getPrevState()[1] - ekf_robot.getPrevState()[1]) ** 2
        ) / (iterations * 1.0)

        angleVelSum += (
            (actual_robot.getPrevState()[5] - ekf_robot.getPrevState()[5]) ** 2
        ) / (iterations * 1.0)
        xVelSum += (
            (actual_robot.getPrevState()[3] - ekf_robot.getPrevState()[3]) ** 2
        ) / (iterations * 1.0)
        yVelSum += (
            (actual_robot.getPrevState()[4] - ekf_robot.getPrevState()[4]) ** 2
        ) / (iterations * 1.0)
