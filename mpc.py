class MPC:
    def __init__(self, path, horizon, dt):
        ...
        self.dt = dt
        self.bnds = jnp.concatenate(
            (
                jnp.array([(-30, 30)] * 4 * horizon),
                jnp.array([(-100, 100)] * 6 * horizon),
            )
        )
    ...
    def objective(self, x, state):
        ...
        for i in range(horizon):
            ...
            target_position = jnp.array(
                [target_position[0], target_position[1], self.path[-1][2]]
            )
        ...
        for i in range(self.horizon):
            ...
            if not (atEnd):
                ...
                target_trajectory.append(
                    current_segment.moveAlongSegment(segment_distance)
                )
    ...
    def step(self, prev_state):
        ...
        eq_constraints = lambda x, prev_state=prev_state: np.sum(
            (x[4 * horizon : 4 * horizon + 6] - prev_state) ** 2
        )
        ...
        x0 = jnp.concatenate(
            (
                jnp.concatenate(
                    (cdr_controls, prev_opt[4 * horizon - 4 : 4 * horizon])
                ),
...
# paths to test on
CYCLE_HUB = np.array(
    [[13.75 / 2, 130, np.pi / 2], [13.75 / 2, 80, np.pi / 2], [35, 65, 3 * np.pi / 4]]
)  # cycle path
CYCLE_WAREHOUSE = np.array(
    [[35, 65, 3 * np.pi / 4], [13.75 / 2, 80, np.pi / 2], [13.75 / 2, 130, np.pi / 2]]
)  # cycle path
